...
def plot_time_series(report_files_per_protocol):
    ...
    fig.suptitle('Evaluasi Kinerja Berdasarkan Waktu Simulasi\n(Menggunakan Data MessageStatsTimeReport)', fontsize=16, fontweight='bold')
    ...
    for i, proto in enumerate(protocols):
        ...
        axs[0, 0].plot(sim_times, delivery, marker=markers[i], color=colors[i], label=proto, linewidth=2, markersize=6)
        axs[0, 1].plot(sim_times, overhead, marker=markers[i], color=colors[i], label=proto, linewidth=2, markersize=6)
        axs[1, 0].plot(sim_times, latency,  marker=markers[i], color=colors[i], label=proto, linewidth=2, markersize=6)
        axs[1, 1].plot(sim_times, dropped,  marker=markers[i], color=colors[i], label=proto, linewidth=2, markersize=6)
...
if __name__ == "__main__":
    print("=== Skrip Plotting Kinerja The ONE Simulator (Parser MessageStatsTimeReport) ===")
    ...
    plot_time_series(report_files_per_protocol)
