...
public class CCRoutingDamar extends QLearningRouter {
	 ...
	 * @param s
	 */
	public CCRoutingDamar(Settings s) {
		super(s);
	 ...
	 * @param r
	 */
	protected CCRoutingDamar(CCRoutingDamar r) {
		super(r);
	...
	private void updateTransitivePreds(DTNHost host) {
		...
		if (!(otherRouter instanceof CCRoutingDamar)) {
			return;
		}

		double pForHost = getPredFor(host); // P(a,b)
		Map<DTNHost, Double> othersPreds =
			((CCRoutingDamar)otherRouter).getDeliveryPreds();
	...
	private double getOtherPredFor(Message m, DTNHost other) {
		...
		if (otherRouter instanceof CCRoutingDamar) {
			return ((CCRoutingDamar)otherRouter).getPredFor(m.getTo());
		}
		...
		if ((SimClock.getTime() - lastUpdateTime) >= updateInterval) {
			...
			for(Map.Entry<Integer,Tuple<DTNHost, List<Integer>>> entry: waitForReward.entrySet()) {
				if(entry.getKey() == newState && entry.getValue().getValue() != null && !entry.getValue().getValue().isEmpty()) {
					...
					CCRoutingDamar othRouter = (CCRoutingDamar) other.getRouter();
	...
	@Override
	public CCRoutingDamar replicate() {
		return new CCRoutingDamar(this);
	}
